...
const TruckForm: React.FC<TruckFormProps> = ({
  ...
  const [isUniqueCodeError, setIsUniqueCodeError] = useState(false);
  ...
  useEffect(() => {
    if (initialData) {
      setInitialStatus(initialData.status || undefined);
      reset(initialData);
    }
  }, [initialData, reset]);
  ...
  useEffect(() => {
    if (
      ...
      setIsUniqueCodeError(true);
    } else {
      setIsUniqueCodeError(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      {!isSuccessCreateTruck && !isSuccessUpdateTruck && (
        <>
          <Controller
            name="code"
            control={control}
          ...
          <div className="block mt-4">
            ...
            {isLoadingUpdateTruck && (
              <span className="ml-2">Updating truck...</span>
            )}
          </div>
        </>
      )}
      {isSuccessCreateTruck && (
        <>
          <p>Truck successfully created.</p>
          <div className="block mt-4">
      ...
      {isSuccessUpdateTruck && (
        <>
          <p>Truck successfully updated.</p>
          <div className="block mt-4">
            <Button variant="primary" onClick={onClose}>
              Ok
            </Button>
      ...
      {(isErrorCreateTruck || isErrorUpdateTruck) && !isUniqueCodeError && (
        <>
